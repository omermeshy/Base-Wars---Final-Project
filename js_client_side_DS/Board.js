class Board
{
    constructor(lay, width, hight, x, y)
    {
        ...
        //flag for important changes that the board have to be update
    	...
    	this.img_planes.SetOnTouchDown(detect_pressed_base);
    	lay.AddChild(this.img_planes);
    }

	//update all the components on the board
	Update()
    {
        //update the score
    ...
    IsAlive()
    {
        ...
        {
            if(this.bases[0].GetOwner() != this.bases[i].GetOwner())
            {
                return "no winner";
            }
        }
    ...
    //return the list of the bases on the board
    GetBases()
    {
        return this.bases;
    }
    ...
    ReleasePlane()
    {
        ...
        //לשלוח ליריב ששוגר מטוס לבסיס חדש
        send_new_plane_launched(player_color, start_base.GetScore(), start_base.GetId(), target_base.GetId());
...
function detect_pressed_base(ev)
{
    var bases = board.GetBases();
